...
namespace Thoth.Managers
{
    internal static class ExampleUsage
    {
        public static void Run()
        {
            ...
            PrintCardsToConsole(personalityCards, zodiacalSunCards);

            bool isCharacterCardPresent = personalityCards.Any(c => c.Role == ArcanaRole.CharacterCard);
            if (!isCharacterCardPresent)
            {
                Console.WriteLine("~~~~~~~~~~~~~~ CHARACTER CARD\nThe same as the Personality Card\n\n");
            }

            // Name cards
            practitioner.SetName(ReadName());
            nameCards = practitioner.GetNameCards();
            PrintCardsToConsole(nameCards);
