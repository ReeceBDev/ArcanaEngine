...
namespace Thoth.Resources.Calculators
{
    internal class AstrologicalCalculator : IAstrologicalCalculator
    {
        ...
        public IEclipticDegree GetAscendantByTime(DateTimeOffset birthTime, double latitude, double longitude)
        {
            ...
            (_, double[] ascmc) = CalculateHouses(julianDay, latitude, longitude);

            // ascmc[0] contains the Ascendant degree (0-359.999). The Ascendant is independent of the house system in use.
            int absoluteDegree = (int)Math.Floor(ascmc[0]);

            return new EclipticDegree(absoluteDegree);
        }

        public ImmutableArray<IEclipticDegree> GetHouseCuspsByTime(DateTimeOffset birthTime, double latitude, double longitude)
        {
            double julianDay = ToJulianDay(birthTime);

            (double[] cusps, _) = CalculateHouses(julianDay, latitude, longitude);

            // cusps[1] through cusps[12] hold the twelve house cusps in ascending zodiacal order; cusps[1] is the 1st house.
            IEclipticDegree[] houseCusps = new IEclipticDegree[12];

            for (int house = 1; house <= 12; house++)
            {
                houseCusps[house - 1] = new EclipticDegree((int)Math.Floor(cusps[house]));
            }
        ...
        /// <summary>
        /// Calculate the houses and angles of a nativity via Swiss Ephemeris, using the fixed whole-sign house system.
        /// </summary>
        private (double[] cusps, double[] ascmc) CalculateHouses(double julianDay, double latitude, double longitude)
        {
            double[] cusps = new double[13]; // 12 house cusps + 1
            double[] ascmc = new double[10]; // Ascendant, MC, etc.
            string serr = "";

            // Calculate houses using the whole-sign ('fixed') house system.
            ...
            if (result < 0)
                throw new InvalidOperationException($"Failed to calculate houses: {serr}");
