...
namespace Thoth.Resources.Json
{
    internal class ThothDeck : IThothDeck
    {
        ...
        private static readonly string MajorArcanaPath = Path.Combine(AppContext.BaseDirectory, "Resources", "Data", "Thoth_MajorArcana");
        private static readonly string MinorArcanaPath = Path.Combine(AppContext.BaseDirectory, "Resources", "Data", "Thoth_MinorArcana");
        ...
        private IArchetype DeserializeArcana<T>(T arcanaIdentity, string rootPath) where T : Enum
        {
            ...
            if (matchingFiles.Length == 0)
                throw new FileNotFoundException($"No file found matching pattern '{searchPattern}' in '{rootPath}'.");
