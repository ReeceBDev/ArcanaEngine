...
namespace Thoth.Types.Practitioner
{
    ...
    internal class CelestialWheel : ICelestialCorrespondences
    {
        ...
        /// <summary> The practitioner's twelve natal houses and their zodiacal correspondences, calculated via the fixed whole-sign house system. </summary>
        public ImmutableArray<IZodiacalHouse>? Houses { get; set; }
        ...
        private ImmutableArray<IZodiacalHouse> GenerateHouses()
        {
            if (!birthTime.HasValue || !birthLocation.HasValue)
                throw new InvalidOperationException("Birth time and location required to generate houses.");

            ImmutableArray<IEclipticDegree> houseCusps = astrologicalCalculator.GetHouseCuspsByTime(birthTime.Value, birthLocation.Value.latitude, birthLocation.Value.longitude);

            ZodiacalHouse[] houses = new ZodiacalHouse[houseCusps.Length];

            for (int i = 0; i < houseCusps.Length; i++)
            {
                houses[i] = new ZodiacalHouse(cardFetcher, i + 1, houseCusps[i]);
            }
        ...
        private void RefreshAllSigns()
        {
            ...
            if (birthLocation.HasValue)
            {
                ...
                //Generate the fixed houses, which use the practitioner's birth location alongside their birth time.
